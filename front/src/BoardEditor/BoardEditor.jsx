...
const BoardEditor = () => {
  ...
  const handleSaveClick = async () => {
    try {
      ...
      if (!boardId || !postDone) {
        method = 'POST'
        url = `${API_BASE}boards/with_zones`
      } else {
        method = 'PUT'
        url = `${API_BASE}boards/with_zones/${boardId}`
      }
  ...
  const HandleListPublicTemplates = async () => {
    try {
      const response = await fetch(`${API_BASE}boards/templates`)
      if (!response.ok) {
  ...
  const getTemplateGameName = async gameId => {
    try {
      const game = await fetch(`${API_BASE}games/${gameId}`)
      if (!game.ok) {
  ...
  const LoadTemplate = async templateId => {
    try {
      const response = await fetch(`${API_BASE}boards/with_zones/${templateId}`)
  ...
  const handleMouseDown = (e, zone) => {
    ...
    const isResize =
      offsetX > zone.width - resizeThreshold &&
      offsetY > zone.height - resizeThreshold
  ...
  const addZonePair = () => {
    ...
    const findNextSuffix = start => {
      let suffix = start
      while (
        existingNames.has(`unnamed zone${suffix === 1 ? '' : '_' + suffix}`)
      ) {
        suffix++
      }
      return suffix
  ...
  const handleDeleteZone = async zoneId => {
    ...
    const confirmDelete = window.confirm(
      'Are you sure you want to delete this zone and its symmetric pair?'
    )
    ...
    const symmetricZone = zones.find(
      z =>
        z.id !== zoneId &&
    ...
    try {
      for (const id of idsToDelete) {
        const response = await fetch(`${API_BASE}boards/zones/${id}`, {
          method: 'DELETE'
        })
